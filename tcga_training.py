...
def train_one(train_loader, input_dim, num_domains, pos_weight_val, domain_class_weights, device, seed, config):
    ...
    model = DANN_MLP(input_dim=input_dim, num_domains=num_domains, dropout_rate=dropout_rate).to(device)
    ...
    for epoch in range(epochs):
        model.train()
        total_loss = 0
        for batch_x, batch_y_stage, batch_y_domain in train_loader:
...
def train_and_evaluate(config_path='config.yaml'):
    ...
    (train_dataset, cal_dataset, test_dataset,
     input_dim, num_domains,
     stage_classes, domain_classes,
     pos_weight_val, domain_class_weights,
     y_stage_cal, y_stage_test, y_domain_test) = load_and_prepare_data(config)
    ...
    for seed in seeds:
        model = train_one(
            train_loader, input_dim, num_domains,
            pos_weight_val, domain_class_weights, device, seed, config
        )
        last_model = model
        model.eval()
        cal_probs = []
        with torch.no_grad():
            for bx, _, _ in cal_loader:
    ...
    with torch.no_grad():
        for bx, _, _ in test_loader:
            _, dl = last_model(bx.to(device), alpha=1.0)
            probs = F.softmax(dl, dim=1)
